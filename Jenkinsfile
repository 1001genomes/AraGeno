        ...
        stage('test') {
            steps {
                script {
                    ...
                    def app_img = docker.build("arageno")

                    // link app container to db container, expose platinum test config path
                    app_img.inside() {
                        // run the behave tests
                        echo "running tests"
                        sh returnStatus: true, script: "cd /srv/web/ && py.test -ra -p no:cacheprovider --junitxml ${env.WORKSPACE}/TESTS-arageno.xml"
                    }
                     // collect unit test results
                    junit allowEmptyResults: true, testResults: "TESTS-arageno.xml"
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    docker.withRegistry('https://docker.artifactory.imp.ac.at', 'jenkins_artifactory_creds') {
                        ...
                        def server_img = docker.build("docker.artifactory.imp.ac.at/the1001genomes/arageno:${env.BUILD_ID}")

                        server_img.push('latest')
                        sshagent(['1001genome_deploy_ssh_key']) {
                            env.DEPLOY_HOST = 'arageno.sci.gmi.oeaw.ac.at'
                            sh '''
